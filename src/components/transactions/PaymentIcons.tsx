...
type PropType = { name: 'visa' | 'paypal' | 'union' | 'master-card' }
const PaymentIcons = ({ name }: PropType): JSX.Element => {
  return (
    <>
      {name === 'visa' ? (
        <Image src={'/visa.png'} alt={'visa logo'} width={69} height={44} />
      ) : null}
      {name === 'master-card' ? (
        <Image src={'/master.png'} alt={'visa logo'} width={65} height={39} />
      ) : null}
      {name === 'union' ? (
        <Image src={'/union.png'} alt={'visa logo'} width={79} height={57} />
      ) : null}
      {name === 'paypal' ? (
        <svg
